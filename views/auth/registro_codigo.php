...
<p>Enviamos um código de 6 dígitos para <strong><?= htmlspecialchars($email ?? '') ?></strong>. Insira abaixo:</p>
<?php if (!empty($devCodigo)): ?>
  <div class="muted" style="margin-bottom:8px;">(Ambiente de desenvolvimento) Seu código é: <strong><?= htmlspecialchars($devCodigo) ?></strong></div>
<?php endif; ?>
<form method="post" action="<?= url('registro/validar-codigo') ?>" class="code-form" autocomplete="off">
  ...
  <div class="muted">Válido por 10 minutos. Verifique a caixa de spam.</div>
</form>
<script>
(function(){
